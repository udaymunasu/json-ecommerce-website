...
export class SellerUpdateProductComponent implements OnInit {
  ...
  submit() {
    ...
    let existingCategories = this.productForm.get('category')?.value;

    if (existingCategories && existingCategories.length > 0) {
      data.category = existingCategories;
    } else {
     data.category = this.categoryItems;
    }
  ...
  patchFormWithProductData() {
    ...
    this.categoryInputArrayMode =
      Array.isArray(this.productData?.category) &&
